...
### (1) 추천 요청 (User → FE → API)
  ...
  * FE는 다음 정보를 API로 전송한다: `doc_id`, `user_id`, `selected_text`, `context: { "prev": "...", "next": "..." }`
  * (context\_hash와 context\_text는 FE가 아닌 백엔드 API 서버가 생성/계산함)
  * 이 요청이 추천 트랜잭션(A)의 시작점이 된다.
  * `recommend_session_id`는 항상 API 서버가 생성한다.
...
| Phase | 사용되는 요소 | 출처 |
| :--- | :--- | :--- |
| **1.0** | **P\_vec, P\_rule** | context\_block(E), Rule Engine |
| **1.5** | + P\_doc | document\_context\_cache(F) |
| **2.0** | + user\_profile 기반 feature | user\_profile(G) |
| **3.0** | + P\_user, + P\_cluster | G(user\_profile), J(cluster\_profile) |
| **4.0** | + style/tone generator | 모든 데이터 기반 |

### (3) Vector Search (P\_vec) — E.context\_block 기반

  * `context_text`를 EmbeddingModel(embedding\_v1)로 임베딩을 생성한 뒤, 해당 벡터로 VectorDB(KNN) 검색을 수행한다.
  * 모든 임베딩은 `embedding_version`을 포함해야 한다.

### (3-1) Rule Engine Execution ($P_{\text{rule}}$) — 규칙 검사기

  * [cite_start]**목적:** 벡터 유사도($P_{\text{vec}}$)만으로는 보장하기 어려운 **정해진 규칙(맞춤법, 사내 스타일 가이드, 금칙어 등)**을 검증하여 점수에 반영한다[cite: 3, 9].
  * **로직:** `context_text`를 입력받아 Regex(정규표현식) 및 Dictionary 매칭을 수행한다.
  * **점수 산출 ($P_{\text{rule}}$):**
      * 위반 사항이 없을 경우: 1.0 (가산점 또는 유지)
      * 규칙 위반 시: 감점(Penalty) 또는 0.0 처리하여 최종 추천 순위에서 배제.
  * **역할:** `P_vec`이 "비슷한 문맥"을 찾는다면, `P_rule`은 "틀리지 않은 표현"을 강제하는 **Hard Constraint** 역할을 수행한다.

### (4) Macro Context 조회 (P\_doc) — F.document\_context\_cache

  * **조회 키:** `doc_id`
  * **반환 값:** `macro_category_hint`, `macro_topic`, `cache_hit`, `last_updated`
  * **무효화 조건:** `diff_ratio >= 10%` (즉각 무효화 및 재계산 요청) 또는 TTL 만료.

### (5) User Profile 조회 (P\_user) — G.user\_profile

  * **조회 키:** `user_id`
  * **반환 값:** `preferred_category_vector`, `user_embedding_v1`, `recommend_accept_rate` 등

### (6) Cluster Profile 조회 (P\_cluster) — J.cluster\_profile

  * **조회 키:** `cluster_id` (user\_profile에서 획득)
  * **반환 값:** `centroid_embedding`, 카테고리·강도·언어 분포 벡터

### (7) 최종 점수 계산 (P\_final)

```python
# v2.4 변경: P_rule 추가 및 동적 가중치 적용
...
```
  ...
  * Phase 1.5 초기나 테스트 시 alpha를 0으로 설정하면, $P_{\text{doc}}$은 계산되지만 $P_{\text{final}}$에는 영향을 주지 않는다. (Shadow Effect)
  * 모든 계산된 점수와 가중치는 \*\*`recommend_log` (I)\*\*에 기록되어야 한다.

### (8) 비동기 데이터 저장

API는 결과를 FE로 반환한 뒤, 비동기 큐를 통해 A, I, E를 저장한다.
...
## 1-4. Storage Layer 상세 설명
  ...
  * **(A/B/C/I/E/H/G 저장소 — BigQuery → MongoDB)**: 대규모 분석 및 ETL 파이프라인의 기반. (v2.4 최적화)
  * **(K — MongoDB 기반 원본 문서 저장)**: 문서 전체 단위 저장, diff 기반 캐시 무효화 정책에 사용.
  * **(D — MongoDB 기반 correction\_history)**: 기업 Ground Truth 저장소. C 이벤트 발생 시 생성.
...
**목표:** Sentencify 시스템의 "기술적 안정성"뿐만 아니라, 수집된 데이터가 기업에 주는 \*\*"비즈니스 가치(Business Value)"\*\*를 시각적으로 증명한다.

#### 3.1 아키텍처 및 원칙 (Sidecar Pattern)

  * **Zero-Touch:** 기존 Backend API 로직을 전혀 수정하지 않고 독립적으로 배포된다.
  * **Read-Only:** 대시보드는 DB(MongoDB, Redis)에 대한 **읽기 권한**만 가진다.
  * **Admin-Only:** 관리자 전용 포트/인증을 통해 접근을 제어한다.
  * **Tech Stack:** Streamlit (Python), Plotly, MongoDB (Direct Query as DWH), Redis.

#### 3.2 Phase별 시각화 명세 (Metrics Specification)

대시보드는 현재 활성화된 Phase에 따라 점진적으로 탭(Tab)이 해금되는 구조를 가진다.

**(1) Phase 1: 서비스 안정성 및 수집 현황 (System Stability)**
...
**(2) Phase 1.5: 지능형 분석 및 적응형 스코어링 (Context Intelligence)**
...
| **Drafting vs Polishing** | 유저 작성 단계 분석 (초안 vs 퇴고) | Histogram | `K.diff_ratio` 분포 (0.1 기준) |
...
**(3) Phase 2: 데이터 자산화 및 품질 관리 (Data Assets)**

  * **핵심 메시지:** "학습 가능한 고품질 데이터(Golden Data)가 자산으로 쌓이고 있습니다."

| 지표명 (Metric) | 설명 | 시각화 방식 | 쿼리 로직 (Pseudo) |
| :--- | :--- | :--- | :--- |
| **Golden Data Count** | 정합성(High-Consistency) 검증 완료 데이터 | Area Chart | `count(H)` where `consistency="high"` |
...
| **Correction Funnel** | 조회(A) → 실행(B) → 선택(C) 전환율 | Funnel Chart | `A` → `B` → `C` 단계별 카운트 |

**(4) Phase 3\~4: 유저 인사이트 및 자동화 성과 (Future Vision)**

  * **핵심 메시지:** "사용자 성향을 이해하고, AI가 업무 시간을 획기적으로 단축합니다."

| 지표명 (Metric) | 설명 | 시각화 방식 | 쿼리 로직 (Pseudo) |
| :--- | :--- | :--- | :--- |
| **User Cluster Map** | 유저 성향 군집 지도 (t-SNE) | 2D Scatter | `G.user_embedding_v1` 시각화 |
| **Personalization Lift** | 개인화 적용 전후 수용률 상승폭 | Bar Chart | `Accept Rate (Phase 2 vs 3)` |
| **Zero-Shot Accept** | 수정 없이 AI 제안 즉시 수락 비율 | Donut Chart | `C` where `is_auto_prompt=True` |
| **ROI / Efficiency** | 토큰 비용 대비 수용 효과 | Line Chart | `Accepted Count` / `Token Usage` |

#### 3.3 구현 가이드 및 디렉토리 구조

  * **데이터 조회 전략:**

      * **실시간성 제어:** `@st.cache_data(ttl=300)` (5분 캐싱)을 사용하여 DB 부하 방지.
      * **대용량 처리:** `H`나 `A` 테이블 조회 시 `aggregate` 파이프라인을 활용하여 DB 레벨에서 연산 후 결과만 전송.
  ...
  * **디렉토리 구조:**
    ...
    sentencify-mvp/
    ├── dashboard/
    │   ├── app.py           # Streamlit 메인 엔트리 (Navigation)
    │   ├── pages/           # Phase별 페이지 분리
    │   │   ├── 1_Overview.py
    │   │   ├── 2_Data_Assets.py
    │   │   └── 3_User_Insights.py
    │   ├── queries/         # DB 조회 로직 분리 (pymongo, redis)
    │   ├── components/      # 재사용 차트 (Funnel, Metrics Card)
    │   ├── requirements.txt
    │   └── Dockerfile
    └── docker-compose.yml   # dashboard 서비스 (Port 8501)
    ```

### 4. Phase 2 완료 기준

* H(학습 데이터) 생성 성공률 **99% 이상**.
* G(유저 프로필) 생성 성공률 **97% 이상**.
* Dashboard에서 `reco_accept_ratio` 및 `macro_hit_rate` 정상 노출.
