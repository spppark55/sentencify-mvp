...
graph TD
  subgraph "실시간 경로 (Real-time Path)"
      ...
      API -- "7. P_final 계산<br>P_final = (1 - alpha) * (w_vec * P_vec + w_rule * P_rule)<br>+ alpha * P_doc + w_user * P_user + ..." --> API

      API -- "8. 추천 결과 반환<br>(insert_id, 추천 옵션)" --> FE
      FE -- "9. 사용자에게 추천 옵션 표시" --> User

      API -- "10. 비동기 이벤트 발행<br>(editor_recommend_options, recommend_log)" --> MQ["메시지 큐<br>(Kafka)"]
  ...
  subgraph "데이터 저장소 (Unified Storage - MongoDB)"
      ...
      Consumer -- "14. 로그 적재 (A/B/C/I)" --> MongoLogs["로그 컬렉션<br>(log_a, log_b, log_i...)"]
      Consumer -- "15. 문서 원본 저장 (K)" --> MongoDocs["문서 컬렉션<br>(full_document_store)"]
  ...
  subgraph "실시간 관제 (ELK Stack)"
      ...
      ES --> Kibana["📈 Kibana Dashboard<br>(Real-time Monitoring)"]
  end
  ...
  subgraph "분석 및 시각화 (Analytics)"
      ...
      MongoLogs -- "20. 데이터 분석/관리" --> AdminDash["📊 Streamlit Admin<br>(Analytics & Inspector)"]
...
**4.1 전체 흐름 (Dual Subscription Architecture)**

API 서버는 Kafka로 이벤트를 발행(Produce)하고, 두 개의 독립적인 Consumer Group이 이를 구독하여 각자의 저장소에 적재합니다.

1.  **Event Production (API → Kafka):**
    *   API는 다음 6가지 이벤트를 Kafka 토픽으로 발행합니다: `A`, `B`, `C`, `E`, `I`, `K`.

2.  **Event Consumption (Kafka → Storage):**
    *   **Path 1: Data Lake (MongoDB & Qdrant)**
        *   **Consumer:** `Python Worker` (Smart Ingester)
        *   **Events:** `A`, `B`, `C`, `E`, `I`, `K` (전체)
        *   **Action:** 모든 로그를 MongoDB에 적재. 
            *   **D 생성:** `C` 이벤트(선택) 수신 시, Consumer가 데이터를 정제하여 `D(Correction History)`를 생성 및 저장 (수락/거절 여부 무관하게 모두 저장).
            *   **E 저장:** `E` 이벤트 수신 시 Qdrant(Vector DB)에 임베딩 적재.
    *   **Path 2: Observability (ELK Stack)**
        *   **Consumer:** `Logstash`
        *   **Events:** `A`, `B`, `C`, `I` (관제용 핵심 로그)
        *   **Action:** 실시간 모니터링을 위해 Elasticsearch에 적재.

3.  **Batch Synchronization (Offline Path):**
    *   **H 생성 (ETL):** `A`, `B`, `C`, `D` 등 모든 로그를 조인하여 학습용 데이터셋 `H(Training Examples)`를 생성 (MongoDB).
    *   **Golden Sync:** 생성된 `H`를 Python Worker가 읽어 Elasticsearch의 Golden Index(`sentencify-golden-*`)로 동기화.
