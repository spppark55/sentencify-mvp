...
st.divider()
st.subheader("📉 Correction Funnel Analysis")

funnel_data = get_correction_funnel()

if funnel_data:
    fig = go.Figure(go.Funnel(
        y=["View (A)", "Run (B)", "Accept (C)"],
        x=[funnel_data["view"], funnel_data["run"], funnel_data["accept"]],
        textinfo="value+percent initial",
        opacity=0.65,
        marker={"color": ["#AB63FA", "#EF553B", "#00CC96"]}
    ))
    fig.update_layout(title_text="User Action Funnel")
